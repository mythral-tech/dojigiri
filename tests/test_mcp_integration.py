...
These test the actual doji_scan/doji_scan_file/doji_fix/doji_explain/doji_analyze_project
...
class TestWizScan:
    ...
    def test_scan_min_severity_filter(self, buggy_py_file):
        ...
        result_critical = doji_scan(buggy_py_file, min_severity="critical")
    ...
    def test_scan_language_filter(self, tmp_path):
        ...
        result = doji_scan(str(tmp_path), language="python")
        assert "Scan:" in result
    ...
    def test_scan_repeated_calls_no_cache_stale(self, buggy_py_file):
        ...
        result1 = doji_scan(buggy_py_file, min_severity="info")
        result2 = doji_scan(buggy_py_file, min_severity="info")
...
class TestWizScanFile:
    ...
    def test_scan_file_clean(self, clean_py_file):
        result = doji_scan_file(clean_py_file)
        assert "File:" in result
        assert "python" in result
...
class TestWizAnalyzeProject:
    ...
    def test_analyze_project_language_filter(self, project_dir):
        result = doji_analyze_project(project_dir, language="python")
        assert "Project:" in result
...
class TestToolSequences:
    def test_scan_then_fix_same_file(self, buggy_py_file):
        ...
        scan_result = doji_scan_file(buggy_py_file)
        assert "No issues found" not in scan_result

        fix_result = doji_fix(buggy_py_file, min_severity="info")
    ...
    def test_scan_then_explain(self, buggy_py_file):
        ...
        scan_result = doji_scan_file(buggy_py_file)
        explain_result = doji_explain(buggy_py_file)
    ...
    def test_scan_dir_then_scan_file(self, tmp_path, buggy_py_file):
        ...
        dir_result = doji_scan(str(tmp_path), min_severity="info")
