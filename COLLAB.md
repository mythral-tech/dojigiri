...
**Last agent**: Claude
**Date**: 2026-03-03
**What they did**: FP reduction round 2 — 4 fixes, 9 new tests, 859 total passing. (1) unused-import: dotted submodule tracking (`import X.Y` used as `X.Y.foo()` now recognized), (2) null-dereference: `self.attr` guard patterns (if self.x is not None, assert self.x, etc.), (3) early-exit guard bug fix: now verifies block body contains raise/return before marking continuation as guarded, (4) unused-variable: TypeVar/NewType/NamedTuple at module scope skipped. Re-benchmarked: 3140 -> 827 total (74% reduction from baseline).

**Previous**: Oz — FP reduction sprint

## Review
**For Oz**: Round 2 FP fixes. Key changes: (1) `detector.py` — dotted import usage check looks at root name for `import X.Y`, (2) `ts_nullsafety.py` — self.attr patterns in all guard categories + early-exit guard now tracks `has_exit` flag, (3) `ts_scope.py` — `_TYPE_DEFINITION_CALLS` tuple for TypeVar/NewType etc. at module scope. 9 new tests in `test_fp_reduction.py`. Remaining FPs are mostly structural (feature-envy, long-method) or require cross-file analysis (module-level objects imported elsewhere).
